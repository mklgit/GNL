...
/*   Updated: 2021/09/28 15:21:04 by mirueta-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*leftxt[1024];
	char		*line;

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (0);
	leftxt[fd] = ft_read_to_leftxt(fd, leftxt[fd]);
	if (!leftxt[fd])
		return (NULL);
	line = ft_getline(leftxt[fd]);
	leftxt[fd] = ft_new_leftxt(leftxt[fd]);
	return (line);
}
